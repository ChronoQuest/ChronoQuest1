    ...
    public struct RewindState
    {
        ...
        public static RewindState Lerp(RewindState a, RewindState b, float t)
        {
            ...
            if (a.CustomData != null || b.CustomData != null)
            {
                ...
                result.SetCustomData("isGrounded",
                    t < 0.5f ? a.GetCustomData<bool>("isGrounded", true)
                            : b.GetCustomData<bool>("isGrounded", true));

                result.SetCustomData("isWallSliding",
                    t < 0.5f ? a.GetCustomData<bool>("isWallSliding", false)
                            : b.GetCustomData<bool>("isWallSliding", false));

                result.SetCustomData("IsFlipped",
                    t < 0.5f ? a.GetCustomData<bool>("IsFlipped", false)
                            : b.GetCustomData<bool>("IsFlipped", false));

                // FlyingEnemy
                result.SetCustomData("FacingDirection",
                    t < 0.5f ? a.GetCustomData<Vector3>("FacingDirection", Vector3.one)
                            : b.GetCustomData<Vector3>("FacingDirection", Vector3.one));

                result.SetCustomData("EnemyState",
                    t < 0.5f ? a.GetCustomData<int>("EnemyState", 0)
                            : b.GetCustomData<int>("EnemyState", 0));

                result.SetCustomData("DetectRange",
                    Mathf.Lerp(
                        a.GetCustomData<float>("DetectRange", 10f),
                        b.GetCustomData<float>("DetectRange", 10f), t));
                ...
                // SlimeEnemy
                result.SetCustomData("flipX",
                    t < 0.5f ? a.GetCustomData<bool>("flipX", false)
                            : b.GetCustomData<bool>("flipX", false));

                result.SetCustomData("midJump",
                    t < 0.5f ? a.GetCustomData<bool>("midJump", false)
                            : b.GetCustomData<bool>("midJump", false));

                result.SetCustomData("frameIndex",
                    Mathf.RoundToInt(Mathf.Lerp(
                        a.GetCustomData<int>("frameIndex", 0),
                        b.GetCustomData<int>("frameIndex", 0), t)));

                // Player cast layer
                result.SetCustomData("Layer1Hash",
                    t < 0.5f ? a.GetCustomData<int>("Layer1Hash", 0)
                            : b.GetCustomData<int>("Layer1Hash", 0));

                result.SetCustomData("Layer1Time",
                    Mathf.Lerp(
                        a.GetCustomData<float>("Layer1Time", 0f),
                        b.GetCustomData<float>("Layer1Time", 0f), t));

                // SpellProjectile
                result.SetCustomData("hasHit",
                    t < 0.5f ? a.GetCustomData<bool>("hasHit", false)
                            : b.GetCustomData<bool>("hasHit", false));

                result.SetCustomData("lifetime",
                    Mathf.Lerp(
                        a.GetCustomData<float>("lifetime", 0f),
                        b.GetCustomData<float>("lifetime", 0f), t));

                // PlayerSpellSystem
                result.SetCustomData("nextFireTime",
                    Mathf.Lerp(
                        a.GetCustomData<float>("nextFireTime", 0f),
                        b.GetCustomData<float>("nextFireTime", 0f), t));

                // Boss projectiles
                result.SetCustomData("IsActive",
                    t < 0.5f ? a.GetCustomData<bool>("IsActive", true)
                            : b.GetCustomData<bool>("IsActive", true));

                result.SetCustomData("GrowSize",
                    Mathf.Lerp(
                        a.GetCustomData<float>("GrowSize", 1f),
                        b.GetCustomData<float>("GrowSize", 1f), t));

                result.SetCustomData("Age",
                    Mathf.Lerp(
                        a.GetCustomData<float>("Age", 0f),
                        b.GetCustomData<float>("Age", 0f), t));

                result.SetCustomData("FullSize",
                    t < 0.5f ? a.GetCustomData<bool>("FullSize", false)
                            : b.GetCustomData<bool>("FullSize", false));

                result.SetCustomData("IsEnding",
                    t < 0.5f ? a.GetCustomData<bool>("IsEnding", false)
                            : b.GetCustomData<bool>("IsEnding", false));

                result.SetCustomData("IsKinematic",
                    t < 0.5f ? a.GetCustomData<bool>("IsKinematic", false)
                            : b.GetCustomData<bool>("IsKinematic", false));

                result.SetCustomData("ColOffset",
                    t < 0.5f ? a.GetCustomData<Vector2>("ColOffset", Vector2.zero)
                            : b.GetCustomData<Vector2>("ColOffset", Vector2.zero));

                // Homing Fireball
                result.SetCustomData("LockedDir",
                    Vector2.Lerp(
                        a.GetCustomData<Vector2>("LockedDir", Vector2.zero),
                        b.GetCustomData<Vector2>("LockedDir", Vector2.zero),
                        t));

                result.SetCustomData("DirLocked",
                    t < 0.5f ? a.GetCustomData<bool>("DirLocked", false)
                            : b.GetCustomData<bool>("DirLocked", false));

                // Trap
                result.SetCustomData("TrapBroken",
                    t < 0.5f ? a.GetCustomData<bool>("TrapBroken", false)
                            : b.GetCustomData<bool>("TrapBroken", false));
                ...
                result.SetCustomData("CycleComplete",
                    t < 0.5f ? a.GetCustomData<bool>("CycleComplete", false)
                            : b.GetCustomData<bool>("CycleComplete", false));
